...
public:
  PatternBindingBuilder (BuilderContext &ctx, PlaceId init, HIR::Type *type)
    : AbstractBuilder (ctx), init (init),
      type (type ? tl::optional (type) : tl::nullopt)
  {}
  ...
  void visit_identifier (const Analysis::NodeMapping &node, bool is_ref)
  {
    ...
    if (init_place.tyty->get_kind () == TyTy::REF)
      {
	init_place.lifetime = ctx.lookup_lifetime (type.map ([] (auto t) {
  ...
  void visit (HIR::ReferencePattern &pattern) override
  {
    ...
    auto ref_type = type.map (
      [] (auto t) { return static_cast<HIR::ReferenceType *> (t); });

    type = ref_type.map ([] (auto r) { return r->get_base_type ().get (); });
  ...
  void visit (HIR::StructPattern &pattern) override
  {
    ...
    auto struct_type = type.map ([] (auto t) {
