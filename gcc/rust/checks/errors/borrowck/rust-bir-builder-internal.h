...
struct BuilderContext
{
  ...
  {
    bool is_loop = false;
    NodeId label = UNKNOWN_NODEID; // UNKNOWN_NODEID if no label
    PlaceId label_var = INVALID_PLACE;
    BasicBlockId break_bb = INVALID_BB;
    BasicBlockId continue_bb = INVALID_BB; // Only valid for loops
  };
...
protected:
  ...
  [[nodiscard]] static NodeId get_nodeid (HIR::Expr &expr)
  {
    return expr.get_mappings ().get_nodeid ();
  }

  [[nodiscard]] static NodeId get_nodeid (HIR::Pattern &pattern)
  {
    return pattern.get_mappings ().get_nodeid ();
  }

  template <typename T>
  [[nodiscard]] TyTy::BaseType *lookup_type (T &pattern) const
  {
  ...
  {
    ctx.get_current_bb ().statements.emplace_back (Node::SWITCH, switch_val);
  ...
  {
    ctx.get_current_bb ().statements.emplace_back (Node::STORAGE_DEAD, place);
  }

  void push_storage_live (PlaceId place)
  {
    ctx.get_current_bb ().statements.emplace_back (Node::STORAGE_LIVE, place);
  }
