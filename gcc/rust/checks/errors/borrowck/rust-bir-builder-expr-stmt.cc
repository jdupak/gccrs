...
ExprStmtBuilder::visit (HIR::FieldAccessExpr &expr)
{
  ...
  autoderef (receiver);
  auto type = ctx.place_db[receiver].tyty;
...
ExprStmtBuilder::visit (HIR::BreakExpr &brk)
{
  ...
  if (brk.has_label ())
	...
	= std::find_if (ctx.loop_and_label_stack.rbegin (),
			...
			[label] (auto &info) { return info.label == label; });
...
ExprStmtBuilder::visit (HIR::ReturnExpr &ret)
{
  ...
  ctx.get_current_bb ().statements.emplace_back (Node::RETURN);
}
...
ExprStmtBuilder::visit (HIR::LetStmt &stmt)
{
  if (stmt.has_init_expr ())
    ...
    {
      // TODO
      rust_sorry_at (stmt.get_locus (), "pattern matching in let stmts");
    }
}
