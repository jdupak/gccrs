...
TypeCheckItem::Resolve (HIR::Item &item)
{
  ...
  auto &vis_item = static_cast<HIR::VisItem &> (item);
  ...
  TyTy::SubstitutionArgumentMappings *infer_arguments)
{
  ...
  *infer_arguments = TyTy::SubstitutionArgumentMappings (
    std::move (args), {}, regions_from_nullable_args (infer_arguments), locus);
...
TypeCheckItem::visit (HIR::StructStruct &struct_decl)
{
  ...
  std::vector<TyTy::StructFieldType *> fields;
  LifetimeParamIdxMap variants_lifetime_params;
  for (auto &field : struct_decl.get_fields ())
    {
      TyTy::BaseType *field_type
	= TypeCheckType::Resolve (field.get_field_type ().get ());
...
TypeCheckItem::visit (HIR::Function &function)
{
  ...
  if (function.has_generics ())
    resolve_generic_params (function.get_generic_params (), substitutions); // TODO resolve constraints
  ...
  auto fn_type = new TyTy::FnType (
    ...
    std::move (substitutions),
    TyTy::SubstitutionArgumentMappings::empty(context->get_lifetime_resolver ().get_num_bound_regions ()),
    region_constraints);
